...
std::optional<Intersection> Scene::traceRay(const Ray& ray) const {
    ...
    for (const auto& primitive : _primitives) {
        ...
        if (intersection && intersection->distance > 0 && intersection->distance < closestDistance) {
...
bool Scene::isInShadow(const Vector3D& point, const std::shared_ptr<ILight>& light) const {
    ...
    for (const auto& primitive : _primitives) {
        ...
        // If we found an intersection that's between the point and the light, we're in shadow
        if (intersection && intersection->distance > 0.001 && intersection->distance < lightDistance) {
            return true;
        }
    }

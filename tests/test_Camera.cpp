...
bool vectorsNearlyEqual(const Vector3D& v1, const Vector3D& v2, double epsilon = 1e-5) {
...
TEST(CameraTest, CenterRayGeneration) {
    ...
    Ray centerRay = camera.generateRay(camera.getWidth() / 2, camera.getHeight() / 2);
...
TEST(CameraTest, CornerRaysGeneration) {
    ...
    Ray bottomRightRay = camera.generateRay(camera.getWidth() - 1, camera.getHeight() - 1);
    ...
    EXPECT_TRUE(vectorsNearlyEqual(bottomRightRay.getOrigin(), Vector3D(0, 0, 0)));
    ...
    Vector3D expectedTopLeft = Vector3D(-aspectRatio * tanHalfFov, tanHalfFov, -1.0).normalized();
    Vector3D expectedTopRight = Vector3D(aspectRatio * tanHalfFov, tanHalfFov, -1.0).normalized();
    Vector3D expectedBottomLeft = Vector3D(-aspectRatio * tanHalfFov, -tanHalfFov, -1.0).normalized();
    Vector3D expectedBottomRight = Vector3D(aspectRatio * tanHalfFov, -tanHalfFov, -1.0).normalized();
    ...
    EXPECT_TRUE(vectorsNearlyEqual(bottomLeftRay.getDirection(), expectedBottomLeft));
    EXPECT_TRUE(vectorsNearlyEqual(bottomRightRay.getDirection(), expectedBottomRight));
}
...
TEST(CameraTest, RotatedCameraRayGeneration) {
    ...
    EXPECT_TRUE(vectorsNearlyEqual(centerRay.getOrigin(), Vector3D(0, 0, 0)));
    EXPECT_TRUE(vectorsNearlyEqual(centerRay.getDirection(), Vector3D(1, 0, 0), 1e-5));
    ...
    EXPECT_TRUE(vectorsNearlyEqual(centerRay.getOrigin(), Vector3D(0, 0, 0)));
    EXPECT_TRUE(vectorsNearlyEqual(centerRay.getDirection(), Vector3D(0, -1, 0), 1e-5));
}
